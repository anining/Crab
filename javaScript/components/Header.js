...
export default function Header ({ scene = { descriptor: { options: {} }, route: { name: '-' } }, previous, navigation = N, replace, headerRight, onPress, style, label, icon, color, backOnPress }) {
    try {
        ...
        // 自定义导航
        if (['GamePage', 'FeedBackPage', 'TaskDetailPage', 'AccountBindPage', 'AccountHomePage', 'FundingRecordsPage', 'WithdrawRecordsPage', 'WithdrawPage', 'PupilInfoPage', 'CardPackagePage', 'DailyMoneyPage', 'DailyRedPackagePage', 'OpenMoneyPage'].includes(scene.route.name)) {
            return <></>;
        }
        return (
